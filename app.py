...
# description
st.markdown('This is a demo that exemplifies how to use LetsData question answering app. '
            'App relies on data, collected and enriched by our team and provides citations for all sources used for '
            'answers. \n'
            'If you are running this app from a mobile device, tap on any empty space to apply changes to input fields.')
    ...
    if formatted_start_date and formatted_end_date:
        ...
        if st.button('RUN SEARCH'):
            ...
            try:
                ...
                content = resp.content.strip('```markdown').strip('```')
                st.markdown(content)
                st.write('******************')
